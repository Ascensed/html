...
muestraPeliculas = (peliculas) => {
    for (let i = 0; i < peliculas.length; i++) {
        if (peliculas[i].clasificacion === 0) {
            ...
            const imagenPelicula = document.getElementById('img' + i);
            const tituloPelicula = document.getElementById('tit' + i);
            imagenPelicula.src = peliculas[i].imagen;
            tituloPelicula.innerHTML = peliculas[i].titulo;
        }
    }
}
...
muestraSeries = (series) => {
    for (let i = 0; i < series.length; i++) {
        if (series[i].clasificacion === 0) {
            const contenedorPelicula = document.getElementById('contenedor-serie');
            contenedorPelicula.classList.add('contenedor-serie');
            const linkPelicula = document.createElement('a');
            linkPelicula.href = '#';
            const divPelicula = document.createElement('div');
            divPelicula.classList.add('pelicula');
            ...
            const imagenPelicula = document.getElementById('img' + i);
            const tituloPelicula = document.getElementById('tit' + i);
            imagenPelicula.src = series[i].imagen;
            tituloPelicula.innerHTML = series[i].titulo;
        }
    }
}
